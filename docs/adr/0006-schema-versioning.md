...
Every JSON file may declare an optional `schemaVersion: string`. The current target lives in `src/data/migrate.ts` as `CURRENT_SCHEMA_VERSION`. A `MIGRATIONS` registry keyed by the *old* version produces the next shape; `migrate()` walks the chain.
