...
function load<T extends z.ZodTypeAny>(schema: T, raw: unknown, file: string): z.infer<T> {
...
function hasItems(id: string): boolean {
  switch (id) {
    ...
    case "experience":
      return experience.visible && visibleOnly(experience.items).length > 0;
    case "projects":
      return projects.visible && visibleOnly(projects.items).length > 0;
    case "skills":
      return skills.visible && visibleOnly(skills.categories).length > 0;
    case "competitive-programming":
      return (
        competitiveProgramming.visible && visibleOnly(competitiveProgramming.platforms).length > 0
      );
    case "education":
      return education.visible && visibleOnly(education.items).length > 0;
    case "publications":
      return publications.visible && visibleOnly(publications.items).length > 0;
    case "certifications":
      return certifications.visible && visibleOnly(certifications.items).length > 0;
    case "achievements":
      return achievements.visible && visibleOnly(achievements.items).length > 0;
    case "timeline":
      return timeline.visible && visibleOnly(timeline.items).length > 0;
    case "open-source":
      return openSource.visible && visibleOnly(openSource.items).length > 0;
    case "talks":
      return talks.visible && visibleOnly(talks.items).length > 0;
    case "awards":
      return awards.visible && visibleOnly(awards.items).length > 0;
    case "blog":
      return blog.visible && visibleOnly(blog.items).length > 0;
    case "startups":
      return startups.visible && visibleOnly(startups.items).length > 0;
    case "products":
      return products.visible && visibleOnly(products.items).length > 0;
    case "patents":
      return patents.visible && visibleOnly(patents.items).length > 0;
    case "mentoring":
      return mentoring.visible && visibleOnly(mentoring.items).length > 0;
    case "media":
      return media.visible && visibleOnly(media.items).length > 0;
    case "testimonials":
      return testimonials.visible && visibleOnly(testimonials.items).length > 0;
    case "contact":
